...
string Token::TokenString[] = {
  "ID", "BOOL", "FLOAT", "INT", "CHAR", // ID and constants
...
Token processAlpha(string lexeme, int lineNumber){
    for(uint8_t i =0; i< sizeof(mykeywords)/sizeof(keyword_token); ++i){
        if(lexeme == mykeywords[i].text){
            if(lexeme == "true") return Token(mykeywords[i].tok_type, lexeme, 1, lineNumber);
            return Token(mykeywords[i].tok_type, lexeme, 0, lineNumber);
        }
    }
